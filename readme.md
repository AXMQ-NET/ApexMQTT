...
| 测试维度 | 性能指标 (Metrics) | 测得峰值 (Peak Performance) | 测试场景 | 备注 |
| :--- | :--- |:------------------------|:------|:---|
| **分发性能** | QoS 0 带宽 | **92.7 MB/s** | N:N多主题并发 | 4,000主题并发，消息吞吐稳定 |
| | QoS 1 带宽 | **64.3 MB/s** | N:N多主题并发 | 领先EMQX 13%，协议效率优势 |
| | QoS 2 带宽 | **41.3 MB/s** | N:N多主题并发 | 领先EMQX 26%，ACK处理优化 |
| **带宽能力** | 网络出站带宽 | **5.81 Gbps** | 1:N扇出测试 | 阿里云内网带宽峰值分发 |
| | 应用层分发速率 | **2.2 GB/s** | 1:N扇出测试 | 全链路零拷贝内存路径 |
| **内存管理** | 1,000,000 连接内存 | **~1.63 GB** | 百万连接测试 | 均摊 1.71KB/连接 (极简设计) |
| | 内存曲线 | **极度平稳** | 百万连接测试 | 自研 Slab Pool 绕过原生 GC |
